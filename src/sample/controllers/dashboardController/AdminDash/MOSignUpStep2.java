...
public class MOSignUpStep2  extends MOSignUpStep1{
    ...
    void backToUserList(ActionEvent event) throws IOException {
        ...
        Parent user = FXMLLoader.load(getClass().getResource("../../../views/dashboard/adminDash/Step2/moDetails/Step2_Doctor.fxml"));
    ...
    void changeToNext2(ActionEvent event) {   // next button on step 1
        ...
        medicalOfficer.setAddress(
                addrFirstText.getText().trim()+
                        ","+addrSecondText.getText().trim()+
                        ","+addrSecondText.getText().trim());

        medicalOfficer.setGender(genderDropDown.getValue());
        medicalOfficer.setMaritalStatus(maritalDropDown.getValue());


        BorderPane tempBorderPane = (BorderPane) signUp2Anchor.getParent();
        tempBorderPane.setCenter(MedicalOfficerDetails.signUpmoMap.get("step3"));
    }
